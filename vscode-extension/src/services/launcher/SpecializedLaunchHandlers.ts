...
export class SpecializedLaunchHandlers {
  ...
  private async _prepareAnalysisFile(
    ...
    if (fs.existsSync(templatePath)) {
      ...
      analysisContent = `# モックアップ解析

あなたはUIモックアップの解析と実装を行うエキスパートです。すべての応答は必ず日本語で行ってください。
...
3. **実装方針の相談（ユーザーの承認を得てから進める）**
   - ユーザーと一緒に実装方針を決める
   - 技術的な選択についてユーザーの確認を得る
   - 実装の優先順位について相談する
...
注意: ユーザーとの議論を経ずに実装を進めないでください。
必ずユーザーの意図を正確に把握し、期待に沿った実装を行ってください。`;
    }
