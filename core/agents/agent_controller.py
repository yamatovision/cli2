...
from core.agents.state.state_tracker import StateTracker
from core.agents.stuck import StuckDetector
...
from core.logger import LOG_ALL_EVENTS
...
from core.events.action.agent import CondensationAction
...
from core.llm.metrics import Metrics, TokenUsage
from core.storage.view import View
...
class AgentController:
    ...
    def _add_system_message(self):
        for event in self.event_stream.search_events(start_id=self.state.start_id):
            if isinstance(event, MessageAction) and event.source == EventSource.USER:
                # Skip system message addition if user message exists before SystemMessageAction
                # This indicates the eventstream existed before system messages were introduced
                # The agent should handle this case gracefully
                return
        ...
        if not self.is_delegate and hasattr(self.agent, 'prompt_manager') and self.agent.prompt_manager:
            ...
            if user_prompt_content:
                from core.events.action import MessageAction
    ...
    async def _handle_observation(self, observation: Observation) -> None:
        ...
        # Simplified observation logging
        if self._pending_action:
            obs_type = type(observation).__name__
            obs_cause = getattr(observation, 'cause', 'None')
            pending_id = getattr(self._pending_action, 'id', 'None')
            self.log('debug', f"Observation received: {obs_type}(cause={obs_cause}) - pending_action_id={pending_id}")
    ...
    def end_delegate(self) -> None:
        ...
        # Log delegate observation for debugging if needed
        self.log('debug', f"AgentDelegateObservation received: {len(content)} chars, {len(delegate_outputs)} outputs")
