...
def get_workspace_mount_path(cfg: OpenHandsConfig) -> str | None:
    """Get workspace mount path from the sandbox.volumes configuration.
    
    Returns the host path that should be mounted as /workspace, or None if not configured.
    """
    if cfg.sandbox.volumes is not None:
        # Split by commas to handle multiple mounts
        mounts = cfg.sandbox.volumes.split(',')
        
        # Find mount that targets /workspace
        for mount in mounts:
...
def finalize_config(cfg: OpenHandsConfig) -> None:
    ...
    # If CLIRuntime is selected, disable browsing for all agents
    ...
    if cfg.runtime and cfg.runtime.lower() == 'cli':
        ...
        logger.openhands_logger.debug(
            'Automatically disabled browsing for all agents '
            'because CLIRuntime is selected and does not support browsing.',
        )
