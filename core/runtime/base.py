...
from core.config import OpenHandsConfig, SandboxConfig
from core.config.mcp_config import MCPConfig, MCPStdioServerConfig
from core.exceptions import AgentRuntimeDisconnectedError, LLMMalformedActionError
from core.error_handler import handle_malformed_action_error
from core.logger import openhands_logger as logger
from core.events import EventSource, EventStream, EventStreamSubscriber
...
from core.events.serialization.action import ACTION_TYPE_TO_CLASS
from extensions.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from extensions.integrations.service_types import AuthenticationError
from core.runtime.plugins import (
    PluginRequirement,
    VSCodeRequirement,
)
from core.runtime.runtime_status import RuntimeStatus
from core.runtime.utils.edit import FileEditRuntimeMixin
from core.runtime.utils.git_handler import CommandResult, GitHandler
...
class Runtime(FileEditRuntimeMixin):
    ...
    async def _handle_action(self, event: Action) -> None:
        ...
        try:
            ...
            # Handle LLMMalformedActionError gracefully to prevent forced termination
            if isinstance(e, LLMMalformedActionError):
                handled_msg = handle_malformed_action_error(e)
                if handled_msg:
                    # This is a recoverable error (like path restriction)
                    self.log('warning', f'Recoverable error: {handled_msg}')
            ...
            # Handle critical errors that should terminate the session
            err_id = ''
            ...
            self.log('error', f'Critical error while running action: {error_message}')
