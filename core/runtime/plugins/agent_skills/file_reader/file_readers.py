...
from core.runtime.plugins.agent_skills.utils.config import (
    _get_max_token,
    _get_openai_api_key,
...
def parse_pdf(file_path: str) -> None:
    ...
    """
    if PyPDF2 is None:
        print(f'[Error: PyPDF2 package not installed, cannot read PDF file {file_path}]')
        return
...
def parse_docx(file_path: str) -> None:
    ...
    """
    if docx is None:
        print(f'[Error: python-docx package not installed, cannot read DOCX file {file_path}]')
        return
...
def parse_latex(file_path: str) -> None:
    ...
    if LatexNodes2Text is None:
        print(f'[Error: pylatexenc package not installed, cannot read LaTeX file {file_path}]')
        return
...
def parse_pptx(file_path: str) -> None:
    ...
    if Presentation is None:
        print(f'[Error: python-pptx package not installed, cannot read PowerPoint file {file_path}]')
        return
    try:
