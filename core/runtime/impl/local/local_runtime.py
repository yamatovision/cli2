...
from core.exceptions import AgentRuntimeDisconnectedError
...
from core.events.serialization import event_to_dict, observation_from_dict
from extensions.integrations.provider import PROVIDER_TOKEN_TYPE
...
from core.runtime.utils import find_available_tcp_port
from core.runtime.utils.command import get_action_execution_server_startup_command
from core.utils.async_utils import call_sync_from_async
...
class LocalRuntime(ActionExecutionClient):
    ...
    async def connect(self) -> None:
        ...
        if self.sid in _RUNNING_SERVERS:
            ...
            self._workspace_mount_path_in_sandbox = server_info.workspace_mount_path
            self.api_url = (
                f'{self.config.sandbox.local_runtime_url}:{self._execution_server_port}'
            )
            ...
            # Set up workspace directory using sandbox.volumes configuration
            ...
            if workspace_mount_path is not None:
                ...
                self._workspace_mount_path_in_sandbox = '/workspace'
                ...
                self._workspace_mount_path_in_sandbox = self._temp_workspace

            logger.info(
                f'Using workspace directory: {self._workspace_mount_path_in_sandbox}'
            )
            ...
            _RUNNING_SERVERS[self.sid] = ActionExecutionServerInfo(
                ...
                workspace_mount_path=self._workspace_mount_path_in_sandbox,
            )
